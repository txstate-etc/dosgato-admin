...
const customConfig = {
  ...
  rules: {
    ...
    'max-lines': 'off', // overkill, big route pages are normal in this app
    ...
    '@typescript-eslint/no-misused-promises': ['error', { checksVoidReturn: false }], // passing an async function where void is expected (e.g. setTimeout) is fine; keep the check that catches promises used as booleans
    '@typescript-eslint/no-non-null-assertion': 'off', // this is silly, it disallows using ! to mark something as non-null
    '@typescript-eslint/only-throw-error': 'off', // sveltekit error() function does not return an Error :/
    ...
    '@typescript-eslint/strict-void-return': 'off', // passing an async function where void is expected (e.g. setTimeout) is fine
    // FORMATTING RULES
...
export default [
  ...
  ...pluginSvelte.configs['flat/recommended'],
  {
    ...
    rules: {
      ...customConfig.rules,
      '@typescript-eslint/no-unnecessary-type-assertion': 'off' // svelte-eslint-parser and svelte-check disagree about types in svelte files, so assertions eslint sees as unnecessary may still be required by svelte-check
    }
  },
  {
    files: ['src/**/*.ts'],
