...
export const load: Load = async input => {
  ...
  if (api.token && isNotBlank(input.url.searchParams.get('url'))) {
    ...
    if (!requestedUrl.startsWith(environmentConfig.renderBase + '/.preview/') && !requestedUrl.startsWith(environmentConfig.renderBase + '/.compare/')) throw error(400, 'This path only helps with preview and compare links.')
