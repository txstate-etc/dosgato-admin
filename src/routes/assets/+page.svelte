...
<ActionPanel actionsTitle={$store.selected.size === 1 ? $store.selectedItems[0].name : 'Assets'} actions={$store.selected.size === 1 ? singlepageactions($store.selectedItems[0]) : multipageactions($store.selectedItems)}>
  ...
  <svelte:fragment slot="preview">
    {#if selectedItem?.kind === 'asset' && selectedItem.box}
      <img src="{environmentConfig.apiBase}/assets/{selectedItem.id}/w/400/{selectedItem.checksum.substring(0, 12)}/{encodeURIComponent(selectedItem.filename)}?admin=1" width={selectedItem.box.width} height={selectedItem.box.height} alt="">
    {/if}
  </svelte:fragment>
</ActionPanel>
