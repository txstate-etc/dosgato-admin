<script lang="ts">
  import { FileIcon, FormDialog, bytesToHuman, Icon, FieldText } from '@dosgato/dialog'
  ...
  import { omit, roundTo } from 'txstate-utils'
  ...
  async function onUploadSaved () {
    uiLog.log({ eventType: 'AssetDetailPage-modal-' + modal, action: 'Success', target: asset.path })
    modal = undefined
    // eslint-disable-next-line svelte/no-reactive-reassign -- intentional local override to refresh asset data after upload
  ...
  async function onMetaSaved () {
    modal = undefined
    // eslint-disable-next-line svelte/no-reactive-reassign -- intentional local override to refresh asset data after editing metadata
    asset = await getAssetDetail(asset.id)
  }
  ...
  function watchForResizes () {
    ...
    timer = setTimeout(async () => {
      ...
      if (image && refreshes <= 12 && asset.resizes.length < Math.floor(Math.log2(image.width) - Math.log2(100) + 1) * 2) {
        // eslint-disable-next-line svelte/no-reactive-reassign -- intentional local override to refresh asset data while resizes generate
        asset = await getAssetDetail(asset.id)
        watchForResizes()
      } else {
        refreshes = 0
  ...
  function onModalEscape () {
    uiLog.log({ eventType: 'AssetDetailPage-modal-' + modal, action: 'Cancel', target: asset.path })
    modal = undefined
  }

  function openModal (m: Modals) {
    uiLog.log({ eventType: 'AssetDetailPage-modal-' + m, action: 'Open', target: asset.path })
    modal = m
  }

  $: assetMetaDetails = omit((asset
    ? {
      ...(asset.box ? { 'Alternate Text': asset.data.meta?.altText || 'Not Provided' } : {}),
      ...(uiConfig.assetMeta?.details ? uiConfig.assetMeta.details(asset.data.meta ?? {}) : asset.data.meta ?? {})
    }
    : {}), 'altText') // Remove 'altText' from details as it's shown separately with a better label
</script>
...
<div class="container">
    ...
    <DetailPanelSection>
      {#if image}
        <div class="image-container">
          ...
          <button type="button" on:click={() => openModal('preview')}><Icon icon={magnifyingGlassPlus} width="1.3em" hiddenLabel="Show image full screen"/></button>
          {#if asset.corrupt}
  ...
  <div class="left-column">
        ...
        <DetailPanelSection>
          {#if assetReferences.length || !assetReferencesIndirect.length}
            <SortableTable items={assetReferences} emptyMessage="Could not find pages directly referencing this asset."
              headers={[
                { id: 'name', label: 'Asset appears directly on', render: item => item.permissions.viewLatest ? `<a href="${resolve('/pages/[id]', { id: item.id })}">${item.path}</a>` : item.path },
                { id: 'view', label: 'Preview', actions: [{ icon: arrowSquareOut, hiddenLabel: 'Preview In New Window', label: 'Preview', onClick: item => { window.open(resolve('/preview') + '?url=' + encodeURIComponent(`${environmentConfig.renderBase}/.preview/${item.permissions.viewLatest || !item.published ? 'latest' : 'public'}${item.path}.html`), '_blank') } }] }
              ]}
            />
          {/if}
          {#if assetReferencesIndirect.length}
            <SortableTable items={assetReferencesIndirect} emptyMessage="Could not find pages referencing this asset's parent folders."
              headers={[
                { id: 'name', label: 'A parent folder appears on', render: item => item.permissions.viewLatest ? `<a href="${resolve('/pages/[id]', { id: item.id })}">${item.path}</a>` : item.path },
                { id: 'view', label: 'Preview', actions: [{ icon: arrowSquareOut, hiddenLabel: 'Preview In New Window', label: 'Preview', onClick: item => { window.open(resolve('/preview') + '?url=' + encodeURIComponent(`${environmentConfig.renderBase}/.preview/${item.permissions.viewLatest || !item.published ? 'latest' : 'public'}${item.path}.html`), '_blank') } }] }
              ]}
            />
          {/if}
    ...
    <!-- eslint-disable-next-line no-constant-binary-expression -- resizes panel intentionally disabled for now -->
    {#if false && (asset.resizes.length || refreshes)}
        <DetailPanel headerColor="#E5D1BD" header="Resizes{refreshes ? ' (loading more...)' : ''}">
          <DetailPanelSection>
            <StyledList>
              {#each asset.resizes as resize (resize.id)}
                <li class="flex-row">
...
{:else if modal === 'preview' && image}
  <Modal escapable on:escape={onModalEscape}>
    <div class="preview" style:max-width="min({image.width * 3}px, {roundTo(90 * image.width / image.height, 4)}dvh)" style:padding-bottom="{roundTo(100 * image.height / image.width, 4)}%">
      ...
      <button type="button" on:click={onModalEscape}><Icon icon={xLight} width="2em" hiddenLabel="Close Full Screen Image"/></button>
    </div>
  </Modal>
{/if}
