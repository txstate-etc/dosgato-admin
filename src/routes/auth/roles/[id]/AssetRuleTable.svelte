...
{#if rules.length}
  <SortableTable cardedOnMobile mobileHeader={(item) => { return item.site ? item.site.name : 'All Sites' }} items={rules}
    headers={[
      ...
      { id: 'delete', label: 'Delete', icon: (item) => { return item.grants.delete ? { icon: checkIcon, hiddenLabel: 'Delete Permitted' } : { icon: minusIcon, hiddenLable: 'Delete not permitted' } } },
      { id: 'undelete', label: 'Restore', icon: (item) => { return item.grants.undelete ? { icon: checkIcon, hiddenLabel: 'Restore Permitted' } : { icon: minusIcon, hiddenLable: 'Restore not permitted' } } },
