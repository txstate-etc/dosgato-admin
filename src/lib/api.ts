...
class API {
  ...
  async init (token: string | undefined) {
    ...
    environmentConfig.assetRegex = new RegExp('^(?:' + environmentConfig.assetLiveBase + ')?(?:/assets|/resize)/([^/]+)/')
    this.ready()
  }
