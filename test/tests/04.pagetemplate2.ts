...
test.describe('pagetemplate2 contents', () => {
  test.describe('standard', () => {
    ...
    test('should be able to add a link', async ({ adminPage }) => {
      ...
      await expect(editFrame.getByRole('link', { name: 'Link Text' }).last()).toBeVisible()
      await expect(editFrame.getByRole('link', { name: 'Link Text' }).last()).toHaveClass('dg-link')
      await expect(editFrame.getByRole('link', { name: 'Link Text' }).last()).toHaveAttribute('href', 'https://www.college.edu/site1.html')
    })
